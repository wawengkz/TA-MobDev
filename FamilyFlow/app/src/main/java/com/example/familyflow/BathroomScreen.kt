...
class BathroomActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            FamilyFlowTheme {
                ...
                suspend fun fetchTasks() {
                    try {
                        ...
                        if (response.isSuccessful) {
                            ...
                            Log.d("BathroomActivity", "Fetched ${tasks.size} tasks")
                            tasks.forEach { task ->
                                Log.d("BathroomActivity", "Task: ${task.name}, Days: ${task.days}")
                            }
                        } else {
                            scope.launch {
                ...
                suspend fun createTask(task: BathroomTask) {
                    try {
                        isLoading = true

                        // Debug: Print days before creating request
                        Log.d("BathroomActivity", "Creating task with days: ${task.days}")

                        // Make sure we're creating a non-empty set
                        if (task.days.isEmpty()) {
                            Log.e("BathroomActivity", "Attempted to create task with empty days")
                            scope.launch {
                        ...
                        // More debugging
                        Log.d("BathroomActivity", "TaskRequest days: ${taskRequest.days}")
                        ...
                        Log.e("BathroomActivity", "Error creating task", e)
                        scope.launch {
    ...
    // Updated Task class for BathroomScreen to include ID from API
    ...
    fun BathroomScreen(
        ...
        onTaskDeleted: (BathroomTask) -> Unit
    ) {
                ...
                Button(
                    onClick = {
                        ...
                        if (isValid) {
                            ...
                            Log.d("AssignBathroomTaskScreen", "Selected days: $selectedDays")
    ...
    fun BathroomScreenPreview() {
        FamilyFlowTheme {
            BathroomScreen(
                ...
                tasks = listOf(BathroomTask(1, "Example Task", setOf("Mon", "Wed"), "Mother")),
                onTaskUpdated = {},
                onTaskDeleted = {}
            )
        }
